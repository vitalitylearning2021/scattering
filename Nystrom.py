...
for i in range(2 * N):          # --- Observation point
    for j in range(2 * N):      # --- Source point
        ...
        A[i, j] = R[i, j] * M1[i, j] + (np.pi / N) * M2[i, j]
...
# CURRENT COMPUTATION #
#######################
J                   = np.linalg.solve(A, Ei) * 2. / (1j * w * mu)
...
#########################
# --- Implements the condition r >= 2 * D^2 / lambd
rFar                = np.max(np.array([2 * max(a, b)**2 / lambd, 10 * lambd, 10 * max(a, b)]))

Es                  = np.zeros(tn.shape, dtype = np.complex128)
for i in range(tn.size):
    Es[i] = - (w * mu0 / 4.) * (2. * np.pi / tn.size) * np.sum(sp.special.hankel2(0, k * np.sqrt((rFar * np.cos(tn[i]) - Xn)**2 + (rFar * np.sin(tn[i]) - Yn)**2)) * J * np.sqrt((dXn[i]**2 + dYn[i]**2)))
